...
export const resetTables = async () => {
    await db.execute(
        sql.raw('TRUNCATE TABLE password_reset, list_item, list, follow, review_vote, review, comment, deal_vote, notification, push_subscription, price_alert, price_quote, tracking_item, media_item, "user" RESTART IDENTITY CASCADE'),
    );
};
